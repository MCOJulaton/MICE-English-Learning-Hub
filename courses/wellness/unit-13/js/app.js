...
const CHECKIN_STORAGE_KEY = 'wellness_u13_checkin';
...
/* ===================== VOICE ENGINE =====================
   Two named characters: Nan (Guest Relations Coordinator, default British
   female voice) and Ms. Suriya (guest, American female voice via the
   'delegate' kind). Same novelty-voice-exclusion + pitch-safety-net
   pattern established for Units 9-12's own VoiceEngine copies. */
...
function renderCover(){
  ...
  <div class="cover">
    ...
    <p>Unit 13: A Guest's Difficult Request. Listen to a guest's request, weigh it against policy, then negotiate a fair outcome as a group of three.</p>
    <div class="signdock">
...
function renderS1(){
  ...
  const guesses = OPENING_SCENARIO.predictGuesses.map((g,i)=>`
  ...
  <h2 class="section-title">A Guest Wants More</h2>
  <p class="section-sub">Before you know the full story, make a prediction.</p>
  <div class="panel">
    <ul style="margin:0 0 0 18px;padding:0;line-height:1.9;font-size:14.5px;color:var(--ink);">${facts}</ul>
    <p style="font-weight:700;color:var(--navy);margin-top:16px;">${OPENING_SCENARIO.predictQuestion}</p>
    <div class="choices" id="predictChoices" style="margin-top:14px;">${guesses}</div>
  ...
  <div class="panel">
    ...
    <p class="section-sub" style="margin-top:4px;">Before the request comes in, listen to today's guest relations briefing. Fill in the table as you listen, then reveal the answers to check yourself.</p>
    <div class="playbar" style="margin-top:16px;">
      ...
      <div style="flex:1;min-width:180px;">
        ...
        <div class="play-sub" id="s1status">Listen for: the time, the task, and where it happens.</div>
      </div>
    ...
    <table class="dictation-table" id="s1table">
      <thead><tr><th>Time</th><th>Task</th><th>Where</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
    ...
    <hr class="hairline">
    <h3 style="font-size:16px;color:var(--navy);">When a Guest Wants an Exception</h3>
    <p style="color:var(--ink);margin-top:8px;line-height:1.6;font-size:14.5px;">Guests are essential to a wellness resort, but their requests don't always fit neatly within their package. When a guest wants something outside her tier, staff must weigh policy, fairness, and the relationship all at once, and still respond warmly.</p>
  </div>`;
}
function wireS1(){
  ...
  predictChoices.addEventListener('click', e=>{
    ...
    const i = +btn.dataset.guess;
    [...predictChoices.children].forEach(b=>b.classList.remove('correct','wrong'));
    const correct = i === OPENING_SCENARIO.predictAnswerIndex;
    btn.classList.add(correct ? 'correct' : 'wrong');
    predictFeedback.className = 'feedback show ' + (correct ? 'good' : 'meh');
    predictFeedback.textContent = correct
      ? "Good prediction! That's exactly what happens. Read on below."
      : "Not quite. Here's what actually happens:";
    revealPanel.style.display = 'block';
    sendGranularRecord('Unit 13: Prediction', {completionStatus:'reached'});
  });
  ...
  scenarioChoices.addEventListener('click', e=>{
    ...
    if(chosen.size >= 2) sendGranularRecord('Unit 13: Opening scenario', {completionStatus:'reached'});
  });
  ...
  const idleStatus = 'Listen for: the time, the task, and where it happens.';
  function play(){
...
function renderS2(){
  ...
  <p class="section-sub">These 10 words come up again and again in this unit. Click a word to see it used in a real guest relations situation.</p>
  <div class="panel">
...
function renderS2b(){
  const stances = STANCE_CHALLENGE.stances.map((s,i)=>`<div class="big-choice" data-i="${i}" style="min-height:80px;"><div class="bc-lbl">${s.text}</div></div>`).join('');
  return `
  <div class="section-eyebrow">Section 3</div>
  <h2 class="section-title">Choose and Defend</h2>
  <p class="section-sub">${STANCE_CHALLENGE.scenario}</p>
  <div class="panel">
    <p style="font-weight:700;color:var(--navy);">${STANCE_CHALLENGE.question}</p>
    <div class="big-choice-grid" id="stanceChoices">${stances}</div>
    <div class="feedback" id="stanceFeedback"></div>
    <textarea id="stanceDefend" class="challenge-textarea" rows="2" placeholder="Defend your position in 1-2 sentences…" style="margin-top:16px;"></textarea>
  </div>`;
}
function wireS2b(){
  const box = document.getElementById('stanceChoices');
  const fb = document.getElementById('stanceFeedback');
  const defendBox = document.getElementById('stanceDefend');
  let chosenIndex = null, hasTyped = false;
  function checkDone(){
    if(chosenIndex !== null && chosenIndex !== STANCE_CHALLENGE.weakIndex && hasTyped){
      markActivityComplete('s2b', {score: STANCE_CHALLENGE.stances[chosenIndex].text});
    }
  }
  box.addEventListener('click', e=>{
    ...
    if(chosenIndex === STANCE_CHALLENGE.weakIndex){
      fb.className = 'feedback show meh';
      fb.textContent = "Avoiding the decision isn't really a position. Try choosing one you can actually defend.";
    } else {
      fb.className = 'feedback show good';
      fb.textContent = 'A defensible position. Now write your reasoning below.';
    }
    checkDone();
  });
...
function renderS4(){
  ...
  <h2 class="section-title">Reading: Handling Difficult Guest Requests</h2>
  <p class="section-sub">Read the article below. Think about how these ideas apply to the decision in Section 9.</p>
  <div class="panel">
...
function renderS5(){
  ...
  <p class="section-sub">Read the situation from Ms. Suriya's request, then pick which kind of language fits it. Get it right and you'll see the exact phrases to use.</p>
  <div class="panel">
...
function wireS5(){
  ...
  choicesEl.addEventListener('click', e=>{
    ...
    if(isCorrect){
      ...
      fb.className = 'feedback show meh'; fb.textContent = "Not quite. Think about what stage of the request this is.";
...
function renderS6(){
  ...
  <h2 class="section-title">Listening: The Request</h2>
  ...
  <div class="panel">
    <div class="playbar">
      ...
      <div style="flex:1;min-width:180px;">
        ...
        <div class="play-sub" id="s6status">Ms. Suriya calls Nan with a request.</div>
      </div>
  ...
  VoiceEngine.onChange(()=>{
    ...
    if(statusEl){
      statusEl.textContent = isPlaying
        ...
        : 'Ms. Suriya calls Nan with a request.';
    }
    if(playBtn){
      playBtn.innerHTML = isPlaying ? icon('stop',{size:20}) : icon('play',{size:20});
      playBtn.title = isPlaying ? 'Stop' : 'Play';
...
function renderS7(){
  ...
  <p class="section-sub">With a partner, discuss: what did Nan do well on the call? What would you have done differently?</p>
  <div class="panel">
    <p style="color:var(--muted);font-size:13.5px;">Talk it through together. This isn't graded, but it's how you build real speaking fluency before the decision task.</p>
    <hr class="hairline">
    <h3 style="font-size:15px;color:var(--navy);">Script Analysis</h3>
    <p style="color:var(--muted);font-size:13px;margin-top:4px;">These are real strategies used by professional guest relations staff worldwide. Click each one once you can point to where it happened in the call.</p>
...
const S6B_STORAGE_KEY = 'wellness_u13_s6b_role';
function renderS6b(){
  const lock = RoleLock.init(S6B_STORAGE_KEY, S6B_ROLES);
  const policyRows = POLICY_CARD.map(p=>`<li>${p}</li>`).join('');
  const publicInfo = `
    <h3 style="font-size:15px;color:var(--navy);">Package Policy (Reference)</h3>
  ...
  if(!lock.myRole){
    ...
    <p class="section-sub">Group work (3 students): the Guest, the Guest Relations Coordinator, and the Spa Manager.</p>
...
function renderS8(){
  ...
  <p class="section-sub">Now work in a group of three. Take turns playing the Guest Relations Coordinator, the Guest, and the Spa Manager.</p>
  <div class="panel">
    <div class="tabs">
      ${tabs}
    </div>
...
/* ===== Flashcard Drill (Remember-level, replaces the crossword slot) ===== */
function renderCrossword(){
  return `
  <div class="section-eyebrow">Section 11</div>
  <h2 class="section-title">Flashcard Drill</h2>
  <p class="section-sub">Flip each card at your own pace. Be honest with yourself.</p>
  <div class="panel">
    <div class="race-progress" id="fcProgress">Card 1 of ${VOCAB.length}</div>
    <div class="sit-card" id="fcCard" style="margin-top:14px;text-align:center;padding:32px 20px;">
      <p id="fcWord" style="font-family:'Oswald';font-size:26px;color:var(--navy);"></p>
      <div id="fcDetail" style="display:none;margin-top:14px;text-align:left;">
        <p id="fcDef" style="color:var(--ink);"></p>
        <p id="fcEx" style="color:var(--orange-deep);font-style:italic;margin-top:8px;"></p>
      </div>
      <button class="reveal-btn" id="fcFlip" style="margin-top:16px;">Show Definition</button>
    </div>
    <div class="choices" id="fcRate" style="margin-top:16px;display:none;grid-template-columns:1fr 1fr;">
      <button class="choice-btn" data-know="yes">I Knew It</button>
      <button class="choice-btn" data-know="no">Still Learning</button>
    </div>
  </div>`;
}
function wireCrossword(){
  ...
  const detailEl = document.getElementById('fcDetail');
  const defEl = document.getElementById('fcDef');
  const exEl = document.getElementById('fcEx');
  const flipBtn = document.getElementById('fcFlip');
  const rateBox = document.getElementById('fcRate');
  function showCard(){
    if(idx >= order.length){
      progressEl.textContent = 'Done';
      wordEl.textContent = `Finished! You knew ${known}/${order.length}.`;
      detailEl.style.display = 'none';
      flipBtn.style.display = 'none';
      rateBox.style.display = 'none';
      markActivityComplete('crossword', {score:`${known}/${order.length} known`});
      return;
    }
    const item = order[idx];
    progressEl.textContent = `Card ${idx+1} of ${order.length}`;
    wordEl.textContent = item.nm;
    defEl.textContent = item.def;
    exEl.textContent = `"${item.ex}"`;
    detailEl.style.display = 'none';
...
function renderPractice(){
  ...
  <p class="section-sub">Evaluate your partner's delivery of the decision. Check off each item as you observe it.</p>
  <div class="panel">
    ${checklist}
  </div>
  <div class="panel">
    <h3 style="font-size:15px;color:var(--navy);">Bonus: Decide Another Case</h3>
    <p style="color:var(--muted);font-size:13px;margin-top:6px;">Choose ONE situation below and decide and justify a response with a partner.</p>
...
function renderS9(){
  ...
  <div class="panel">
    <div class="email-template">
      <p>Dear Ms. Suriya,</p>
      <textarea id="s9writing" class="challenge-textarea" rows="5" style="margin-top:14px;" placeholder="Write your 4–6 sentence decision email here…"></textarea>
      <p style="margin-top:24px;">We truly value your partnership.</p>
      <p style="margin-top:10px;">Best regards,<br>Guest Relations Team</p>
    </div>
...
function wireS9(){
  ...
  box.addEventListener('input', ()=>{
    ...
    if(len >= 40){
      fb.className = 'feedback show good';
      fb.textContent = 'Nice work. That reads like a real decision email.';
...
function renderComplete(){
  ...
  <div class="cover complete-cover">
    <div class="cover-badge">UNIT 13 COMPLETE</div>
    <h1>You can <span>make the call.</span></h1>
    <p>Keep practicing weighing your options before deciding, and remember: firm, fair, and diplomatic.</p>
