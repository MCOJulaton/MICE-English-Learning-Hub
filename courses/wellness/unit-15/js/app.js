...
const CHECKIN_STORAGE_KEY = 'wellness_u15_checkin';
...
/* ===================== VOICE ENGINE =====================
   Two named characters: Fern (wellness coordinator, default British female
   voice) and Beam (colleague, American male voice). Same novelty-voice-
   exclusion + pitch-safety-net pattern established for Units 9-14's own
   VoiceEngine copies. */
...
function renderCover(){
  ...
  <div class="cover">
    ...
    <h1>One guest. <span>One whole day. You design it all.</span></h1>
    <p>Unit 15: Designing a Guest's Wellness Day. Bring together everything from Units 9 to 14: craft the welcome, the greeting, the recovery, and the farewell, as one connected experience.</p>
    <div class="signdock">
      <div class="signchip"><span class="arrow">→</span> 1 Guest</div>
      <div class="signchip"><span class="arrow">→</span> 4 Touchpoints</div>
      <div class="signchip"><span class="arrow">→</span> Personalize</div>
      <div class="signchip"><span class="arrow">→</span> Integrate It All</div>
    </div>
    <button class="startbtn" onclick="goNext()">Begin the design →</button>
  </div>`;
}
...
function renderS1(){
  ...
  <h2 class="section-title">One Guest, One Whole Day</h2>
  <p class="section-sub">The brief just landed. Read it, then decide what you'd do.</p>
  <div class="panel">
  ...
  <div class="panel">
    <h3 style="font-size:16px;color:var(--navy);">Let's Start the Design</h3>
    <p class="section-sub" style="margin-top:4px;">Before you build the day yourself, listen to today's overview of its stages. Fill in the table as you listen, then reveal the answers to check yourself.</p>
    <div class="playbar" style="margin-top:16px;">
      <button class="play-btn" id="s1play" title="Play">${icon('play',{size:20})}</button>
      <div style="flex:1;min-width:180px;">
        <div class="play-label">PLAY TODAY'S OVERVIEW</div>
        <div class="play-sub" id="s1status">Listen for: the stage, the touchpoint, and why it matters.</div>
      </div>
    ...
    <table class="dictation-table" id="s1table">
      <thead><tr><th>Stage</th><th>Touchpoint</th><th>Why It Matters</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
    ...
    <hr class="hairline">
    <h3 style="font-size:16px;color:var(--navy);">Everything You've Learned, One Day</h3>
    <p style="color:var(--ink);margin-top:8px;line-height:1.6;font-size:14.5px;">This is the final unit before the real final exam. It doesn't teach a brand new skill, it asks you to combine every skill from Units 9 to 14 into one connected, original piece of work: a guest's whole day, designed by you.</p>
  </div>`;
}
function wireS1(){
  ...
  scenarioChoices.addEventListener('click', e=>{
    ...
    if(chosen.size >= 2) sendGranularRecord('Unit 15: Opening scenario', {completionStatus:'reached'});
  });
  ...
  const idleStatus = 'Listen for: the stage, the touchpoint, and why it matters.';
  function play(){
...
function renderS2(){
  ...
  <p class="section-sub">These 10 words come up again and again in this unit. Click a word to see it used in a real day-design situation.</p>
  <div class="panel">
...
function renderS2b(){
  ...
  const cards = CLIENT_REQUESTS.map((r,i)=>`
    <div class="sit-card" data-req="${i}">
      <p style="color:var(--ink);">${r.text}</p>
      <div class="choices" style="margin-top:10px;grid-template-columns:repeat(auto-fit,minmax(110px,1fr));">
        ${stageKeys.map(k=>`<button class="choice-btn" data-stage="${k}" style="padding:10px 12px;font-size:13.5px;">${STAGE_LABELS[k]}</button>`).join('')}
      </div>
    </div>`).join('');
  return `
  <div class="section-eyebrow">Section 3</div>
  <h2 class="section-title">Sort the Requests</h2>
  <p class="section-sub">The client's notes are all mixed together. Sort each one into the day stage it belongs to.</p>
  <div class="panel">
    ${cards}
...
function wireS2b(){
  ...
  document.querySelectorAll('#app [data-req]').forEach(card=>{
    const i = +card.dataset.req;
    const req = CLIENT_REQUESTS[i];
    ...
    box.addEventListener('click', e=>{
      ...
      if(btn.dataset.stage === req.stage){
        btn.classList.add('correct');
        fb.className = 'feedback show good';
        fb.textContent = `Correct, that belongs in ${STAGE_LABELS[req.stage]}.`;
        sorted.add(i);
      } else {
        btn.classList.add('wrong');
        fb.className = 'feedback show meh';
        fb.textContent = 'Not quite that stage. Try again.';
      }
      if(sorted.size >= CLIENT_REQUESTS.length) markActivityComplete('s2b', {score:`${sorted.size}/${CLIENT_REQUESTS.length} sorted`});
    });
  });
}
...
function renderS4(){
  ...
  <h2 class="section-title">Reading: Designing a Complete Wellness Day</h2>
...
function renderS5(){
  ...
  <p class="section-sub">Read where Ms. Renner is in her day, then pick which kind of language fits that moment. Get it right and you'll see the exact phrases to use.</p>
  <div class="panel">
...
function wireS5(){
  ...
  choicesEl.addEventListener('click', e=>{
    ...
    if(isCorrect){
      ...
      btn.classList.add('wrong');
      fb.className = 'feedback show meh'; fb.textContent = "Not quite. Think about where she is in her day.";
...
function renderS6(){
  ...
  <h2 class="section-title">Listening: Planning the Day</h2>
  ...
  <div class="panel">
    <div class="playbar">
      ...
      <div style="flex:1;min-width:180px;">
        ...
        <div class="play-sub" id="s6status">Fern and Beam plan Ms. Renner's day.</div>
      </div>
  ...
  VoiceEngine.onChange(()=>{
    ...
    if(statusEl){
      statusEl.textContent = isPlaying
        ...
        : "Fern and Beam plan Ms. Renner's day.";
    }
    if(playBtn){
      playBtn.innerHTML = isPlaying ? icon('stop',{size:20}) : icon('play',{size:20});
      playBtn.title = isPlaying ? 'Stop' : 'Play';
...
function renderS7(){
  ...
  <p class="section-sub">With a partner, discuss: what did Fern and Beam do well? What would you have done differently?</p>
  <div class="panel">
    <p style="color:var(--muted);font-size:13.5px;">Talk it through together. This isn't graded, but it's how you build real speaking fluency before the design task.</p>
    <hr class="hairline">
    <h3 style="font-size:15px;color:var(--navy);">Script Analysis</h3>
    <p style="color:var(--muted);font-size:13px;margin-top:4px;">These are real strategies used by professional wellness coordinators worldwide. Click each one once you can point to where it happened in the call.</p>
...
/* ===== Section 6b: Build the Day =====
   CREATE task: four labeled prompts, each with its own textarea, so
   students write an original message for each touchpoint before checking
   a full model day. This is Unit 15's distinct mechanic and the
   course's final integration task. */
function renderS6b(){
  const stageBlocks = JOURNEY_STAGES.map(s=>`
    <div class="sit-card">
      <h3 style="font-size:15px;color:var(--navy);">${s.label}</h3>
      <p style="margin-top:6px;color:var(--ink);font-size:14px;">${s.prompt}</p>
      <textarea class="challenge-textarea" id="journey_${s.key}" rows="2" placeholder="${s.placeholder}" style="margin-top:10px;"></textarea>
    </div>`).join('');
  return `
  <div class="section-eyebrow">Section 9</div>
  <h2 class="section-title">Build the Day</h2>
  <p class="section-sub">Work with a partner. Write your own original message for each of the four touchpoints below, for VIP guest Ms. Renner.</p>
  <div class="panel">
    ${stageBlocks}
    <hr class="hairline">
    <button class="reveal-btn" id="journeyReveal">Show a model day</button>
...
function renderS8(){
  ...
  <h2 class="section-title">Speaking Practice: Perform the Day</h2>
  <p class="section-sub">Practice the full day in pairs. Take turns playing the Wellness Coordinator and Ms. Renner.</p>
  <div class="panel">
    <div class="tabs">
      ${tabs}
    </div>
    ${panels}
    <p style="color:var(--muted);font-size:12.5px;margin-top:14px;">Perform the whole day once using your own written messages from Section 9. Then try again with less support, in your own words.</p>
    <hr class="hairline">
...
function renderCrossword(){
  ...
  <div class="panel">
    <div class="race-progress" id="capProgress">Unit ${CAPSTONE_REVIEW[0].unit} · Item 1 of ${CAPSTONE_REVIEW.length}</div>
    <p id="capDef" style="font-weight:700;color:var(--navy);font-size:17px;margin-top:14px;"></p>
    <div class="choices" id="capChoices" style="margin-top:14px;grid-template-columns:1fr 1fr;"></div>
    <div class="feedback" id="capFeedback"></div>
  </div>`;
}
function wireCrossword(){
  let idx = 0, correct = 0;
  const progressEl = document.getElementById('capProgress');
  const defEl = document.getElementById('capDef');
  const choicesEl = document.getElementById('capChoices');
  const fb = document.getElementById('capFeedback');
  function showItem(){
    if(idx >= CAPSTONE_REVIEW.length){
      progressEl.textContent = 'Done';
      defEl.textContent = `Finished! ${correct}/${CAPSTONE_REVIEW.length} correct. Seven units, one course.`;
      choicesEl.innerHTML = '';
      fb.className = 'feedback';
      markActivityComplete('crossword', {score:`${correct}/${CAPSTONE_REVIEW.length}`});
      return;
    }
    const item = CAPSTONE_REVIEW[idx];
    progressEl.textContent = `Unit ${item.unit} · Item ${idx+1} of ${CAPSTONE_REVIEW.length}`;
    defEl.textContent = item.def;
    choicesEl.innerHTML = item.options.map((o,i)=>`<button class="choice-btn" data-i="${i}">${o}</button>`).join('');
    fb.className = 'feedback';
  }
  choicesEl.addEventListener('click', e=>{
    const btn = e.target.closest('.choice-btn'); if(!btn) return;
    ...
    const item = CAPSTONE_REVIEW[idx];
    const i = +btn.dataset.i;
    [...choicesEl.children].forEach((b,bi)=>{ if(bi===item.correct) b.classList.add('correct'); });
    if(i === item.correct){
      correct++;
      fb.className = 'feedback show good';
      ...
      btn.classList.add('wrong');
      fb.className = 'feedback show meh';
      fb.textContent = `Not quite. The answer was "${item.options[item.correct]}".`;
    }
    setTimeout(()=>{ idx++; delete choicesEl.dataset.locked; showItem(); }, 1200);
  });
  showItem();
}
...
function renderPractice(){
  ...
  <p class="section-sub">Evaluate your partner's day design. Check off each item as you observe it.</p>
  <div class="panel">
    ${checklist}
  </div>
  <div class="panel">
    <h3 style="font-size:15px;color:var(--navy);">Bonus: Design Another Day</h3>
    <p style="color:var(--muted);font-size:13px;margin-top:6px;">Choose ONE situation below and design the touchpoints with your partner.</p>
...
function renderS9(){
  ...
  <div class="panel">
    <div class="email-template">
      <p>My chosen touchpoint: ___________________</p>
...
function wireS9(){
  ...
  box.addEventListener('input', ()=>{
    ...
    if(len >= 40){
      fb.className = 'feedback show good';
      fb.textContent = 'Nice work. That reads like a real touchpoint message.';
...
function renderS10(){
  ...
  <div class="panel">
    ${rows}
    <hr class="hairline">
    <p style="font-family:'Oswald';color:var(--navy);font-size:15px;letter-spacing:.03em;">You've now practiced applying, analyzing, evaluating, and creating original Wellness Tourism English across Units 9 to 15. Well done.</p>
  </div>`;
}
function wireS10(){
...
function renderComplete(){
  ...
  <div class="cover complete-cover">
    <div class="cover-badge">UNIT 15 COMPLETE</div>
    <h1>You can <span>design the whole day.</span></h1>
    <p>You've reached the end of the Units 9-15 series. Keep practicing personalizing every message, and remember: it's one connected day, not separate tasks.</p>
