...
const CHECKIN_STORAGE_KEY = 'wellness_u14_checkin';
...
/* ===================== VOICE ENGINE =====================
   Two named characters: Nueng (therapist, default British female voice)
   and Orn (spa manager, American female voice via 'delegate'). Same
   novelty-voice-exclusion + pitch-safety-net pattern established for
   Units 9-13's own VoiceEngine copies. */
...
function renderCover(){
  ...
  <div class="cover">
    ...
    <h1>One guest. <span>Three audiences. Three different messages.</span></h1>
    <p>Unit 14: A Guest Needs Help. Work in a group, listen to the briefing, and craft the right message for the guest, her companion, and the internal log, not one message for everyone.</p>
    <div class="signdock">
      <div class="signchip"><span class="arrow">→</span> 1 Situation</div>
    ...
    <button class="startbtn" onclick="goNext()">The treatment pauses →</button>
  </div>`;
}
...
function renderS1(){
  ...
  const alertHtml = OPENING_SCENARIO.alertLines.map(l=>`<div class="ca-line">${l}</div>`).join('');
  ...
  <h2 class="section-title">A Guest Needs Help</h2>
  ...
  <div class="panel">
    ...
    <p class="section-sub" style="margin-top:4px;">Before any real incident happens, listen to today's readiness briefing. Fill in the table as you listen, then reveal the answers to check yourself.</p>
    ...
    <hr class="hairline">
    <h3 style="font-size:16px;color:var(--navy);">When a Guest Needs Help</h3>
    <p style="color:var(--ink);margin-top:8px;line-height:1.6;font-size:14.5px;">A real incident at a wellness resort tests everything at once: calm care, quick thinking, and communication to several different people, all within minutes. This unit is your biggest challenge yet.</p>
  </div>`;
}
function wireS1(){
...
function renderS2(){
  ...
  <p class="section-sub">These 10 words come up again and again in this unit. Click a word to see it used in a real response situation.</p>
  <div class="panel">
...
function wireS2b(){
  ...
  box.addEventListener('click', e=>{
    ...
    if(chosenIndex === CRISIS_DECISION.weakIndex){
      ...
      fb.textContent = 'Ignoring a worried companion rarely helps, and it can feel evasive. Try a response you can defend.';
    } else {
...
function renderS4(){
  ...
  <h2 class="section-title">Reading: Responding to a Guest Incident</h2>
...
function renderS6(){
  ...
  <h2 class="section-title">Listening: The Response Briefing</h2>
  ...
  <div class="panel">
    <div class="playbar">
      ...
      <div style="flex:1;min-width:180px;">
        ...
        <div class="play-sub" id="s6status">Nueng and Orn coordinate the response.</div>
      </div>
  ...
  <div class="panel">
    ...
    <p class="section-sub" style="margin-top:4px;">Orn now makes a brief PA announcement to the spa lounge. This is a single speaker, not a two-way call.</p>
    <div class="playbar" style="margin-top:14px;">
      ...
      <div style="flex:1;min-width:180px;">
        ...
        <div class="play-sub" id="bcStatus">Orn addresses the spa lounge.</div>
      </div>
...
function wireS6(){
  ...
  VoiceEngine.onChange(()=>{
    ...
    if(statusEl){
      statusEl.textContent = (isPlaying && activeSource === 'dialogue')
        ...
        : 'Nueng and Orn coordinate the response.';
    }
    if(playBtn){
    ...
    if(bcStatusEl){
      bcStatusEl.textContent = (isPlaying && activeSource === 'broadcast') ? 'Playing…' : 'Orn addresses the spa lounge.';
    }
    if(bcPlayBtn){
  ...
  function playBroadcast(){ activeSource = 'broadcast'; VoiceEngine.speakLine(BROADCAST.text, 'delegate'); }
...
function renderS7(){
  ...
  <p class="section-sub">With your group, discuss: what did Nueng and Orn do well? What would you have done differently?</p>
  <div class="panel">
    ...
    <p style="color:var(--muted);font-size:13px;margin-top:4px;">These are real strategies used by professional wellness teams worldwide. Click each one once you can point to where it happened in the call.</p>
...
/* ===== Section 6b: Three Messages, One Guest (Crisis Timeline) =====
   ...
   Round 4 (the next-morning check-in) only unlocks once Round 3 is
   ...
   their story consistent as the situation resolves. State is persisted in
...
const S6B_STORAGE_KEY = 'wellness_u14_s6b_timeline';
...
function renderS6b(){
  ...
  <h2 class="section-title">Three Messages, One Guest</h2>
  <p class="section-sub">Group work. The situation unfolds in 4 rounds. Discuss each one together, choose the strongest message, and keep your story consistent as it unfolds.</p>
  <div class="panel">
    ${blocks}
  </div>`;
}
...
function renderS8(){
  ...
  <p class="section-sub">Work in a group of 3. Each person takes one role below and delivers it in sequence: the Therapist reassures the guest first, the Spa Manager updates the companion next, then the Duty Supervisor reports the incident log.</p>
  <div class="panel">
    <div class="tabs">${tabs}</div>
    ${panels}
...
function renderPractice(){
  ...
  <div class="panel">
    <h3 style="font-size:15px;color:var(--navy);">Bonus: A New Situation</h3>
...
function wireS9(){
  ...
  box.addEventListener('input', ()=>{
    ...
    if(len >= 40){
      fb.className = 'feedback show good';
      fb.textContent = 'Nice work. That reads like a real guest-care message.';
...
function renderComplete(){
  ...
  <div class="cover complete-cover">
    ...
    <h1>You can <span>lead with calm care.</span></h1>
