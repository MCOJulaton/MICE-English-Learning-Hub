...
/* ===================== VOICE ENGINE =====================
   ...
   character. Each voice is chosen to match BOTH the requested accent
   locale AND the speaking character's actual gender (see GENDER in
   data.js), so "Mai" always gets a female-sounding voice and "David"
   always gets a male-sounding voice, never the wrong gender just to
   hit an accent. Falls back gracefully: gender+locale, gender+same
   language, locale only, then any English voice, so it still works on
   a device missing a specific accent. */
...
function renderCover(){
  ...
  <div class="cover">
    ...
    <h1>Wellness Tourism <span>Integrated Listening Challenge.</span></h1>
    <p>Units 7 and 8: Spa &amp; Wellness Services, and Symptoms &amp; Health Conditions. Complete this after your Kahoot or Blooket review.</p>
...
function renderDiscuss(){
  ...
  <h2 class="section-title">Wellness Services and Guest Needs</h2>
...
/* ---- Listening Challenge (one listening per group) ---- */
let listenState = { plays:0 };
function listenReset(){ listenState = { plays:0 }; }

function currentListening(){
  if(!Progress.groupId) return null;
  const group = GROUPS.find(g=>g.id===Progress.groupId);
  return group ? group.listening : null;
}
...
function renderComplete(){
  ...
  <div class="cover complete-cover">
    ...
    <p>You listened carefully, understood a guest's needs, and made a wellness recommendation as a team. Ready for whole-class feedback? Your teacher will take it from here.</p>
...
document.getElementById('btnReset').addEventListener('click', ()=>{
  if(current === RENDERERS.findIndex(r=>r.r===renderListen)) listenReset();
  renderAll();
});
