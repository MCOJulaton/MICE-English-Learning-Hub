...
const CHECKIN_STORAGE_KEY = 'wellness_u12_checkin';
...
function renderCover(){
  ...
  <div class="cover">
    ...
    <h1>Two packages. <span>Which one do you recommend, and why?</span></h1>
    <p>Unit 12: Choosing the Right Package. Compare two real wellness packages with a partner, weigh the trade-offs, and justify your recommendation.</p>
    <div class="signdock">
      <div class="signchip"><span class="arrow">→</span> 2 Packages</div>
...
function renderS1(){
  ...
  <p class="section-sub">It's decision day. Read the guest's message, then decide what you'd do.</p>
  <div class="panel">
  ...
  <div class="panel">
    ...
    <p style="color:var(--ink);margin-top:8px;line-height:1.6;font-size:14.5px;">Choosing a package for a wellness guest is rarely as simple as picking the lowest price. A good recommendation weighs several factors together and can be explained clearly to a guest who is trusting your judgment.</p>
  </div>`;
}
function wireS1(){
...
function renderS4(){
  ...
  <h2 class="section-title">Reading: Comparing Packages</h2>
...
/* ===== Section 6b: Compare and Decide =====
   ...
   partner commits to one package once; from then on only that package's
   ...
   They describe their own package out loud, listen to their partner's,
   then both converge on the same recommendation-writing step. */
const S6B_STORAGE_KEY = 'wellness_u12_s6b_role';
function renderS6b(){
  ...
  const convergence = `
    ...
    <textarea id="recBox" class="challenge-textarea" rows="3" placeholder="We recommend Package ___ because…"></textarea>
  ...
  if(!lock.myRole){
    ...
    <p class="section-sub">Pair speaking. Student A has Package A's full proposal. Student B has Package B's.</p>
    ${RoleLock.renderPicker(S6B_STORAGE_KEY, S6B_ROLES, "Which package's proposal did your teacher assign you?")}`;
  }
...
function renderS8(){
  ...
  <h2 class="section-title">Speaking Practice: Negotiate the Recommendation</h2>
  <p class="section-sub">Practice the negotiation in pairs. Student A is the wellness coordinator; Student B is the guest pushing back on the price. Switch roles for Round 2.</p>
  <div class="panel">
...
function renderS9(){
  ...
  <div class="panel">
    <div class="email-template">
      ...
      <p style="margin-top:10px;">Best regards,<br>Wellness Office Team</p>
    </div>
...
function wireS9(){
  ...
  box.addEventListener('input', ()=>{
    ...
    if(len >= 40){
      ...
      fb.textContent = 'Nice work. That reads like a real recommendation email.';
