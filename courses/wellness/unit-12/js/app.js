...
function renderS6b(){
  ...
  <p class="section-sub">Pair speaking. Student A has Package A's full proposal. Student B has Package B's. Only one screen should be visible per student.</p>
  <div class="panel">
    <div class="ab-toggle">
      <button class="ab-btn active" data-ab="A">Show Student A: Package A</button>
      <button class="ab-btn" data-ab="B">Show Student B: Package B</button>
    </div>
...
function wireS6b(){
  ...
  document.getElementById('recBox').addEventListener('input', function(){
    if(this.value.trim().length >= 10 && seenRoles.size >= 2){
      markActivityComplete('s6b', {score:'both packages viewed'});
    }
  });
}
