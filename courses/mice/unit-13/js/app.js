...
const CHECKIN_STORAGE_KEY = 'mice_u13_checkin';
...
/* ===================== VOICE ENGINE =====================
   Two named characters: Nan (Sponsorship Coordinator, default British
   female voice) and Ms. Preecha (Sponsor B representative, American
   female voice via the 'delegate' kind). Same novelty-voice-exclusion +
   pitch-safety-net pattern established for Units 9-12's own VoiceEngine
   copies. */
...
function renderS1(){
  const rows = WARMUP_SCHEDULE.map((w,i)=>`
    <tr>
      ...
      <td><input type="text" class="dictation-input" data-dict="${i}-f3" placeholder="where"></td>
    </tr>`).join('');
  ...
  <h2 class="section-title">Two Sponsors, One Entrance</h2>
  ...
  <div class="panel">
    ...
    <p class="section-sub" style="margin-top:4px;">Before the request comes in, listen to today's sponsor relations briefing. Fill in the table as you listen, then reveal the answers to check yourself.</p>
  ...
  <div class="feedback" id="s1nudge"></div>
    ...
    <hr class="hairline">
    <h3 style="font-size:16px;color:var(--navy);">When Sponsors Disagree</h3>
    <p style="color:var(--ink);margin-top:8px;line-height:1.6;font-size:14.5px;">Sponsors are essential to MICE events, but their requests don't always fit neatly together. When two sponsors want the same thing, the event team must weigh contracts, policy, and relationships all at once, and still respond within a day.</p>
  </div>`;
}
function wireS1(){
...
function renderS2(){
  ...
  <p class="section-sub">These 10 words come up again and again in this unit. Click a word to see it used in a real sponsor relations situation.</p>
  <div class="panel">
...
function renderS4(){
  ...
  <h2 class="section-title">Reading: Handling Difficult Sponsor Requests</h2>
...
function renderS5(){
  ...
  <p class="section-sub">The phrases sponsor relations staff use, organized by moment.</p>
  <div class="panel">
  ...
  <div class="panel">
    <div class="playbar">
      ...
      <div style="flex:1;min-width:180px;">
        ...
        <div class="play-sub" id="s6status">Ms. Preecha calls Nan with a request.</div>
      </div>
  ...
  VoiceEngine.onChange(()=>{
    ...
    if(statusEl){
      statusEl.textContent = isPlaying
        ...
        : 'Ms. Preecha calls Nan with a request.';
    }
    if(playBtn){
      playBtn.innerHTML = isPlaying ? icon('stop',{size:20}) : icon('play',{size:20});
      playBtn.title = isPlaying ? 'Stop' : 'Play';
...
function renderS7(){
  ...
  <div class="panel">
    ...
    <p style="color:var(--muted);font-size:13px;margin-top:4px;">These are real strategies used by professional sponsor relations staff worldwide. Click each one once you can point to where it happened in the call.</p>
...
function renderS6b(){
  ...
  const convergence = `
    ...
    <div class="feedback" id="decisionNudge"></div>
...
function wireS6b(){
  ...
  decisionReveal.addEventListener('click', ()=>{
    const nudge = document.getElementById('decisionNudge');
    if(justifyBox.value.trim().length < 10){
      nudge.className = 'feedback show meh';
      nudge.textContent = 'Write what you agreed to first (at least a sentence).';
      return;
    }
    nudge.className = 'feedback';
...
/* ===== Flashcard Drill (Remember-level, replaces the crossword slot) =====
   Self-paced flip-and-recall — a different rhythm from the timed/sequential
   quiz formats used in other units. No timer, no penalty; the student's own
   honest self-assessment ("I Knew It" / "Still Learning") is the point. */
...
function renderS9(){
  ...
  <div class="panel">
    <div class="email-template">
      ...
      <p style="margin-top:10px;">Best regards,<br>Sponsorship Team</p>
    </div>
